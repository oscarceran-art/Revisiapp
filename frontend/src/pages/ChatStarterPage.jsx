  ...
  return (
    <div className="min-h-screen pt-20 md:pt-12 px-4 sm:px-6 md:px-10 pb-16" data-testid="chat-starter-page">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <div className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground">New chat</div>
          ...
          {MODES.map(m => {
            ...
            return (
              <button
                ...
                className={`text-left p-4 rounded-2xl border transition-all ${active ? "bg-foreground text-primary-foreground border-foreground" : "bg-background border-border hover:border-foreground/30"}`}
              >
                <Icon size={20} weight="regular" />
                <div className="font-bold text-base mt-2">{m.label}</div>
                <div className={`text-xs mt-1 leading-snug ${active ? "text-primary-foreground/70" : "text-muted-foreground"}`}>{m.desc}</div>
              </button>
            );
          })}
        </div>
        ...
        {/* Subject selector */}
        <div className="mb-6">
          <label className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground block mb-2">Subject context (optional)</label>
          <select
            value={subjectId}
            onChange={e => setSubjectId(e.target.value)}
            className="w-full sm:w-auto border border-border rounded-2xl px-4 py-2.5 bg-background text-sm focus:outline-none focus:border-foreground"
        ...
        {mode === "group" && selected.length < 2 && (
          <div className="text-xs text-muted-foreground mt-2">Pick at least 2 personas for a group chat.</div>
        )}
      </div>
    </div>
  );
