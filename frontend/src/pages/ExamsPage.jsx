...
function CountdownBadge({ days }) {
  ...
  let label, tone;
  if (days < 0) { label = `${Math.abs(days)}d ago`; tone = "bg-black/10 text-muted-foreground"; }
  ...
  else if (days <= 30) { label = `${days} days`; tone = "bg-foreground text-primary-foreground"; }
  else { label = `${days} days`; tone = "bg-foreground/85 text-primary-foreground"; }
  ...
  return (
    <div className="min-h-screen pt-20 md:pt-12 px-4 sm:px-6 md:px-10 pb-16" data-testid="exams-page">
      <div className="max-w-4xl mx-auto">
        <button onClick={() => navigate("/")} className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1.5 mb-6">
        ...
        <div className="flex items-end justify-between mb-8 flex-wrap gap-3">
          <div>
            ...
            <p className="text-muted-foreground mt-2 text-sm sm:text-base">Add every exam — we'll keep the days ticking down and build a revision plan whenever you're ready.</p>
          </div>
          <button
        ...
        {showForm && (
          <div className="glass-card rounded-3xl p-5 sm:p-7 mb-8 space-y-4 animate-fade-up">
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground block mb-2">Exam name</label>
                <input
                  value={form.name}
                  onChange={e => setForm(v => ({ ...v, name: e.target.value }))}
                  placeholder="e.g. Biology Paper 1"
                  className="w-full border border-border rounded-2xl px-4 py-2.5 focus:outline-none focus:border-foreground text-sm"
                  data-testid="exam-name"
                />
              </div>
              <div>
                <label className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground block mb-2">Date & time</label>
                <input
                  type="datetime-local"
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-border rounded-2xl px-4 py-2.5 bg-background text-sm focus:outline-none focus:border-foreground"
              ...
              <div>
                <label className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground block mb-2">Location (optional)</label>
                <input
                  value={form.location}
            ...
            <div>
              <label className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground block mb-2">Notes</label>
              <textarea
                value={form.notes}
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => setShowForm(false)} className="border border-border rounded-2xl px-4 py-2.5 text-sm font-bold hover:bg-black/[0.04] dark:hover:bg-white/[0.06]">Cancel</button>
              <button
        ...
        {upcoming.length === 0 && past.length === 0 && (
          <div className="text-center glass-card rounded-3xl py-16 px-6" data-testid="exams-empty">
            ...
            <p className="text-muted-foreground mt-2 max-w-md mx-auto text-sm">Add your first one to start the countdown.</p>
          </div>
        )}

        {upcoming.length > 0 && (
          <div className="mb-10">
            <div className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground mb-3">Upcoming</div>
            <div className="grid gap-3">
              {upcoming.map(e => {
                const days = daysBetween(e.exam_date);
                return (
                  <div key={e.id} className="glass-card rounded-2xl sm:rounded-3xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4" data-testid={`exam-${e.id}`}>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <CountdownBadge days={days} />
                        {e.subject_name && <span className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">{e.subject_name}</span>}
                      </div>
                      <div className="text-base sm:text-xl font-extrabold mt-1.5 sm:mt-2 truncate">{e.name}</div>
                      <div className="text-[12px] sm:text-[13px] text-muted-foreground mt-1 flex flex-wrap items-center gap-2 sm:gap-3">
                      ...
                      {e.notes && <p className="text-[12px] sm:text-[13px] text-muted-foreground mt-1.5 sm:mt-2 leading-relaxed">{e.notes}</p>}
                    </div>
                    <div className="flex sm:flex-col gap-1.5 sm:gap-2 shrink-0 flex-wrap">
                      ...
                      {e.completed && e.debrief_session_id ? (
                        <button
                          ...
                          className="border border-border rounded-xl sm:rounded-2xl px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm flex items-center justify-center gap-1 hover:bg-black/[0.04] dark:hover:bg-white/[0.06]"
                      ...
                      <button
                        ...
                        className="border border-border rounded-xl sm:rounded-2xl px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm flex items-center justify-center gap-1 hover:bg-black/[0.04] dark:hover:bg-white/[0.06] text-muted-foreground"
        ...
        {past.length > 0 && (
          <div>
            <div className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground mb-3">Past</div>
            <div className="grid gap-3">
              {past.map(e => (
                <div key={e.id} className="glass-card rounded-3xl p-4 flex items-center gap-3 flex-wrap" data-testid={`exam-past-${e.id}`}>
                  <CountdownBadge days={daysBetween(e.exam_date)} />
                  <div className="flex-1 min-w-0">
                    <div className="font-bold truncate">{e.name}</div>
                    <div className="text-[12px] text-muted-foreground">{formatDate(e.exam_date)}</div>
                  </div>
                  {e.completed && e.debrief_session_id ? (
                    <button
                      onClick={() => navigate(`/chat/${e.debrief_session_id}`)}
                      className="border border-border rounded-2xl px-3 py-1.5 text-xs font-bold flex items-center gap-1.5 hover:bg-black/[0.04] dark:hover:bg-white/[0.06]"
                  ...
                  <button onClick={() => handleDelete(e.id, e.name)} className="text-muted-foreground hover:text-foreground p-1" aria-label="Delete"><Trash size={14} /></button>
                </div>
              ))}
            </div>
          </div>
