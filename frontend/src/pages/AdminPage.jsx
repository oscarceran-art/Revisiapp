...
function TokenBar({ used, limit }) {
  if (!limit) return <span className="text-[12px] text-muted-foreground">Unlimited</span>;
  const pct = Math.min(100, Math.round((used / limit) * 100));
  const colour = pct > 90 ? "bg-red-500" : pct > 70 ? "bg-amber-400" : "bg-foreground";
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 bg-foreground/10 rounded-full overflow-hidden" style={{ minWidth: 60 }}>
  ...
  return (
    <span className="flex items-center gap-1">
      <input
        ...
        className="w-24 text-[13px] px-2 py-0.5 rounded-lg border border-foreground/20 outline-none"
      />
      <button onClick={() => { onSave(Number(val)); setEditing(false); }} className="p-1 hover:bg-black/[0.06] dark:hover:bg-white/[0.06] rounded-lg">
        <Check size={13} weight="bold" className="text-green-600" />
      </button>
      <button onClick={() => setEditing(false)} className="p-1 hover:bg-black/[0.06] dark:hover:bg-white/[0.06] rounded-lg">
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10 page-fade">
      ...
      <div className="flex items-center gap-3 mb-8">
        <button onClick={() => navigate("/")} className="p-2 rounded-2xl hover:bg-black/[0.06] dark:hover:bg-white/[0.06] transition-colors">
        ...
        <div>
          ...
          <p className="text-[13px] text-muted-foreground">Manage accounts & usage limits</p>
        </div>
        <div className="ml-auto flex items-center gap-2">
          <button onClick={load} className="flex items-center gap-1.5 px-3 py-2 rounded-2xl border border-border text-[13px] font-semibold hover:bg-black/[0.04] dark:hover:bg-white/[0.06] transition-colors">
        ...
        <div className="glass-card rounded-3xl p-6 mb-6">
          ...
          <div className="flex gap-3 flex-wrap">
            <input
              ...
              className="px-4 py-2.5 rounded-2xl text-[14px] border border-border outline-none focus:border-foreground/40 bg-background"
            />
            <input
            ...
            <button onClick={() => setShowCreate(false)} className="px-4 py-2.5 rounded-2xl border border-border text-[14px] font-semibold hover:bg-black/[0.04] dark:hover:bg-white/[0.06]">
              Cancel
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={s.label} className="glass-card rounded-3xl px-5 py-4">
      ...
      <div className="glass-card rounded-3xl overflow-hidden">
        <div className="px-6 py-4 border-b border-border flex items-center gap-2">
          <Users size={16} weight="regular" className="text-muted-foreground" />
          <span className="text-[14px] font-bold">{users.length} account{users.length !== 1 ? "s" : ""}</span>
        </div>

        {loading ? (
          <div className="px-6 py-12 text-center text-[14px] text-muted-foreground">Loading…</div>
        ) : users.length === 0 ? (
          <div className="px-6 py-12 text-center text-[14px] text-muted-foreground">No accounts yet</div>
        ) : (
              ...
              <div key={u.id} className="px-6 py-4 flex items-start gap-4 flex-wrap">
                ...
                <div className="w-36 shrink-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {u.is_admin && <span className="text-[10px] bg-foreground text-primary-foreground rounded-full px-1.5 py-0.5 font-bold">Admin</span>}
                    {!u.is_active && <span className="text-[10px] bg-red-100 text-red-600 rounded-full px-1.5 py-0.5 font-bold">Disabled</span>}
                  </div>
                  <div className="text-[11px] text-muted-foreground mt-0.5">Last login: {formatDate(u.last_login)}</div>
                </div>

                {/* Token usage */}
                <div className="flex-1 min-w-[180px]">
                  <div className="text-[11px] uppercase tracking-[0.15em] text-muted-foreground mb-1">Daily</div>
                  <TokenBar used={u.tokens_used_today} limit={u.token_limit_daily} />
                  <div className="text-[11px] uppercase tracking-[0.15em] text-muted-foreground mt-2 mb-1">Weekly</div>
                  <TokenBar used={u.tokens_used_week} limit={u.token_limit_weekly} />
                </div>

                {/* Limits editor */}
                <div className="min-w-[160px]">
                  <div className="text-[11px] uppercase tracking-[0.15em] text-muted-foreground mb-1">Daily limit</div>
                  <EditCell value={u.token_limit_daily} onSave={v => update(u.id, { token_limit_daily: v })} />
                  <div className="text-[11px] uppercase tracking-[0.15em] text-muted-foreground mt-2 mb-1">Weekly limit</div>
                  <EditCell value={u.token_limit_weekly} onSave={v => update(u.id, { token_limit_weekly: v })} />
                  <div className="text-[11px] text-muted-foreground mt-1">0 = unlimited</div>
                </div>
                ...
                <div className="flex flex-col gap-1.5 shrink-0">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[12px] font-semibold border border-border hover:bg-black/[0.04] dark:hover:bg-white/[0.06] transition-colors"
                  >
                    {u.is_active
                      ? <><ToggleRight size={14} weight="fill" className="text-green-600" /> Disable</>
                      : <><ToggleLeft size={14} className="text-muted-foreground" /> Enable</>}
                  </button>
                  <button
                  ...
                  {u.username !== user?.username && (
                    <button
                      onClick={() => remove(u)}
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[12px] font-semibold text-red-600 border border-red-200 hover:bg-red-50 dark:hover:bg-red-950 transition-colors"
                    >
